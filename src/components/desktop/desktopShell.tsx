...
export default function DesktopShell({ preload }: Props) {
    ...
    useEffect(() => {
        const handler = (e: Event) => {
            ...
            if (ce.detail?.title) {
                setArchiveTitle(ce.detail.title);
            }
        };
        window.addEventListener("archive-title", handler);
        return () => window.removeEventListener("archive-title", handler);
    }, []);
    ...
    return (
        <div className="desktop-root">
            <div className="icon-grid">
                <Icon label="Projects" iconSrc="/smart-folder.png" onClick={() => open('archive', { payload: preload.archive, pathOverride: '/archive?page=1&category=Projects' })} />
            ...
            {windows.map((w) => {
                const storeWin = wins[w.id as keyof typeof wins];
                const title =
                    w.id === "archive"
                        ? archiveTitle || storeWin?.title || w.title || w.id
                        : storeWin?.title || w.title || w.id;
